...
public class Rules extends CustomCommand {

    public Rules() { super("rules","links"); }

    @Override
    public void execute(CommandSender sender, String label, String[] args) {
        Player player = (Player) sender;
        if (args.length == 0) sendLinksSetting(player, "rules");
        else sendHelpMessage(player, message("rulesCommandUsage"), "/rules");
    }
}
